...
| `vendor/openfortivpn-bundle/`    | Generated by `scripts/windows/Stage-Openfortivpn.ps1` from a Cygwin `openfortivpn` + `ppp` install | Embedded into `SuperManager.msi` as `bin\openfortivpn-bundle\`. CI installs Cygwin + the package, the script copies `openfortivpn.exe` + `pppd.exe` + the cyg*.dll closure into the bundle directory, generates `installer/wix/openfortivpn-files.wxi` listing each file as a WiX component. |
| `vendor/openfortivpn.exe` (legacy) | User-supplied static binary | Embedded directly into the MSI's `bin\` if you have a statically-linked Windows port of openfortivpn. Optional - the bundle path above is the canonical one. |
...
14. ✅ ~~Build openfortivpn in CI via the Cygwin port + bundle the closure into the MSI. FortiGate **SSL VPN** now works out-of-the-box on a fresh install.~~

        ...
        generate_btn.connect_clicked(move |_| {
...

            {
                ...
                rt.spawn(async move {
                    ...
                    *slot
                        .lock()
                        .unwrap_or_else(std::sync::PoisonError::into_inner) = Some(config_text);
                });
            }
...
fn generate_html_report(state: &WizardState, config: &str) -> String {
    ...
    let date_str = now
        .format("%Y-%m-%d %H:%M")
        .map_or_else(|_| "Unknown date".into(), |s| s.to_string());
...
async fn send_provisioning_subscription(prompt: &str, model: &str) -> anyhow::Result<String> {
    let full_prompt = format!("{PROVISIONING_SYSTEM_PROMPT}\n\n---\n\nUser request:\n{prompt}");

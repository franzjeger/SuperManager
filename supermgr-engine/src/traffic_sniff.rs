...
pub async fn analyse_pcap(pcap_path: &Path, evidence_dir: &Path) -> Result<TrafficAuditResult> {
    ...
    for ((src_ip, _proto_id), cluster) in clusters {
        ...
        let _ = writeln!(evidence_body,
            "Cleartext {} from {} — {} event(s)",
            proto.name,
            src_ip,
            cluster.len()
        );
        evidence_body.push_str("Generated by SuperManager traffic-sniff\n");
        let _ = write!(evidence_body, "Source pcap: {}\n\n", pcap_path.display());
        for ev in &cluster {
            let _ = writeln!(evidence_body,
                "[{}] {}:{} -> {}:{}",
                ev.timestamp, ev.src_ip, ev.src_port, ev.dst_ip, ev.dst_port,
            );
            let _ = write!(evidence_body, "    {}\n\n", ev.redacted_excerpt);
        }
...
fn extract_http_form_password(payload: &str) -> Option<String> {
    ...
    let _ = write!(redacted, "sha256:{}", short_hash(password));

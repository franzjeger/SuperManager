...
/// How much it matters that a control fails.
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "lowercase")]
pub enum Severity {
    /// Worth reporting, costs nothing when it fails.
    Info,
    /// Weakens defence in depth; fix when convenient.
    Low,
    /// A control an auditor will ask about.
    Medium,
    /// Directly exploitable, or the difference between contained and not.
    High,
    /// Fix before the next change window.
    Critical,
}
...
#[serde(rename_all = "lowercase")]
pub enum Channel {
    /// Read the value from the REST API. Preferred: faster and needs no shell.
    Api,
    /// Read it by parsing `show` output over SSH. Necessary for settings the
    /// API does not expose.
    Cli,
}
...
pub enum Expectation {
    ...
    NotEqual {
        /// Value the setting must differ from.
        value: String,
    },
    /// Raw value (after trim, case-insensitive) must equal exact.
    Equal {
        /// Value the setting must match exactly.
        value: String,
    },
}
...
pub struct CheckDefinition {
    /// Stable identifier, e.g. `linux.ssh.root-login-disabled`. History
    /// correlates runs by this, so renaming one orphans its past results.
    pub id: String,
    /// One-line name, shown as the row title.
    pub title: String,
    /// What the check is for and why it matters. Author-time text about
    /// intent, distinct from the failure message a run produces.
    pub description: String,
    /// Grouping label the UI sorts by, e.g. `SSH`, `Firewall`.
    pub category: String,
    /// How bad it is when this fails. Drives the score penalty.
    pub severity: Severity,
    /// Benchmark this control comes from, named precisely enough to look up —
    /// including a version, since section numbers move between them.
    pub framework: String,
    /// Section within `framework`, when the control genuinely has one. `None`
    /// where the check is the benchmark's *intent* without being one of its
    /// numbered controls; a guessed number here is a false citation in a
    /// report an auditor reads.
...
pub enum Status {
    ...
    Pass,
    /// The control is not satisfied. Counts against the score.
    Fail,
    /// Not applicable to this device, so deliberately not evaluated.
    Skip,
    /// Could not be evaluated — command failed, value unparseable. Distinct
    /// from `Fail` on purpose: "we could not tell" is not "it is wrong".
    Error,
}

/// What one control evaluated to during a run.
///
/// Carries copies of the definition's title, category and severity so a
/// stored run renders on its own — the library it was evaluated against may
/// have changed, or contain user-supplied checks the reader does not have.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct CheckResult {
    /// The `CheckDefinition::id` this result is for. The join key between a
    /// run and the library — a report resolves description and remediation
    /// through it, so a result with an id no longer in the library renders
    /// bare.
    pub check_id: String,
    /// Outcome of this one control.
    pub status: Status,
    ...
    pub severity: Severity,
    /// Definition's title, copied so a run renders without the library.
    pub title: String,
    /// Definition's category, copied for the same reason. Stamped by the same
    /// helper the library uses, so the two cannot disagree.
    pub category: String,
}

/// Why a run happened. Shown in history so a surprising result can be traced
/// to an operator, the scheduler, or a deploy.
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum TriggerKind {
    /// An operator pressed Run scan.
    Manual,
    /// The periodic sweep. Skipped for hosts scanned recently.
    Scheduled,
    /// Fired after a provisioning run, to catch a deploy that regressed a
    /// control.
    PostDeploy,
}
...
/// One complete evaluation of a baseline against one host.
///
/// The unit of persistence and the unit of comparison: drift is computed
/// between consecutive runs for the same host.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ComplianceRun {
    /// Identifier for this run. Also its filename on disk.
    pub id: String,
    /// Host the run was executed against.
    pub host_id: String,
    /// When the run began.
    pub started_at: DateTime<Utc>,
    /// When it finished. Drift compares consecutive runs by this ordering.
    pub finished_at: DateTime<Utc>,
    /// Device firmware as reported at run time, when known. Recorded because a
    /// control that regressed across a firmware upgrade is a different story
    /// from one an operator changed.
    pub firmware: Option<String>,
    /// Device model, when known.
    pub model: Option<String>,
    /// Hostname at run time. Kept on the run so a report identifies the
    /// device without resolving the host record, which may since have been
    /// renamed or deleted.
    ...
    pub score: u8,
    /// Count of `Status::Pass`.
    pub passed: u32,
    /// Count of `Status::Fail`.
    pub failed: u32,
    /// Count of `Status::Error` — could not be evaluated.
    pub errored: u32,
    /// Count of `Status::Skip` — not applicable to this device.
    pub skipped: u32,
    /// Every control evaluated, in definition order.
    pub checks: Vec<CheckResult>,
}

/// A run without its per-check detail, for the history list.
///
/// Exists so listing thirty runs does not deserialise thirty full check
/// vectors to render thirty rows.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct RunSummary {
    /// Run identifier, used to fetch the full run on demand.
    pub id: String,
    /// When it began — what the history list sorts by.
    pub started_at: DateTime<Utc>,
    /// 0–100.
    pub score: u8,
    /// Count of passing controls.
    pub passed: u32,
    /// Count of failing controls.
    pub failed: u32,
    /// Count that could not be evaluated.
    pub errored: u32,
    /// Firmware at run time, when known.
    pub firmware: Option<String>,
    /// What caused the run.
    pub triggered_by: TriggerKind,
...
/// Write a run to disk under the host's runs directory.
///
/// Called before responding to the caller, so a GUI that dies between the scan
/// and the reply does not lose the result.
pub fn persist_run(run: &ComplianceRun) -> Result<()> {
...
/// Per-host outcome of a fleet-wide scan.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ScanAllResult {
    /// Host this entry is about.
    pub host_id: String,
    /// Human-readable label, so a failure names the device rather than a UUID.
    pub host_label: String,
    /// Run produced, when the scan succeeded.
    pub run_id: Option<String>,
    /// Score of that run.
    pub score: Option<u8>,
    /// Why this host was not scanned. Set instead of `run_id` — one
    /// unreachable device must not fail the whole sweep.
...
/// How one control moved between two runs.
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum DriftKind {
...
/// One control's movement between two runs.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct DriftEntry {
    /// Control that moved.
    pub check_id: String,
    /// Its title, copied for rendering.
    pub title: String,
    /// Its category, copied for grouping.
    pub category: String,
    /// Its severity, copied so the list can sort by consequence.
    pub severity: Severity,
    /// Which direction it moved.
    pub kind: DriftKind,
    /// Status in the earlier run. `None` when the control is new.
    pub previous_status: Option<Status>,
    /// Status in this run. `None` when the control was removed.
    pub current_status: Option<Status>,
    /// Earlier detail text, so a report can show what changed and not only
    /// that something did.
...
/// What changed between a run and the one before it on the same host.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct DriftReport {
    /// Run being reported on.
    pub current_run_id: String,
    /// Run it was compared against. `None` for a host's first run, where
    /// everything currently failing is reported as newly failing.
    pub previous_run_id: Option<String>,
    /// Score of the current run.
    pub current_score: u8,
    /// Score of the previous run.
    pub previous_score: Option<u8>,
    /// Current minus previous. Signed: a regression is negative.
    pub score_delta: i32,
    /// Controls that started failing since the previous run — the section an
    /// operator reads first.
    pub newly_failing: Vec<DriftEntry>,
    /// Controls fixed since the previous run.
    pub newly_passing: Vec<DriftEntry>,
    /// Controls failing in both. Not news, still a problem.
    pub still_failing: Vec<DriftEntry>,
    /// Controls that could not be evaluated in either run.

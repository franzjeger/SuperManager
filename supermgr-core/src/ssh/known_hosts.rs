...
pub struct KnownHostsStore {
    path: PathBuf,
    /// One cache shared by this daemon's SSH sessions. The lock covers both
    /// verification and durable enrollment so competing handshakes cannot
    /// replace each other's first-use pins. Independently opened stores do
    /// not share a cache; run a single daemon per data directory.

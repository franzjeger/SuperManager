...
## Batch 6 — source-built macOS runtime and recoverable system-package candidate

### Changes

- Added an isolated source builder for Bash, OpenSSL, OpenVPN 2, WireGuard tools/Go and monolithic strongSwan. Archives are hash-pinned before extraction. The output contains only selected runtime components/configuration/licenses; static crypto and protected compiled paths avoid Homebrew library/plugin dependencies. Bash default executable/loadable-builtin search paths and wg-quick's interpreter/config/PATH defaults are patched explicitly with upstream-layout assertions.
- Added a reviewed-file manifest validator: exact file set/SHA-256, bounded size/count, no symlinks/special or group/world-writable files, native architecture, and restricted Mach-O dependency paths. No unresolved rpaths or Homebrew references are accepted. Pinned source archives/build trees are retained outside the repository for release provenance.
- Added signed system-package construction. The supplied helper must already match the pinned identity with hardened runtime and no debug/injection entitlements. Runtime Mach-O files receive release signatures; the final signed payload is hashed again. A Developer ID Installer signature is required; there is no unsigned distribution fallback.
- Package scripts enforce architecture, protected directories, inactive VPN processes, a monotonic root-owned build counter and an exclusive publication lock. The build floor is checked again under the lock. Helper/runtime/plist backups and a pending transaction journal support rollback on launch failure and explicit recovery after installer-process termination. Same-build replacement, downgrades and silent adoption of old manual installations are rejected.
- Removed GUI administrator-shell fallback and automatic helper binary replacement. Disabled the development helper-copy and NOPASSWD-enablement scripts. Existing installed sudoers exceptions are **not** removed automatically. The GUI's readiness check now requires an authorized version RPC and helper policy version 2. System Component Setup reports signed-release/package/runtime requirements; the helper exposes explicit runtime availability and unsupported-profile diagnostics.
- Release builds fail early for missing/invalid runtime inputs or signing identities and propagate Xcode failure instead of reusing an existing app. The signed system package replaces the old DNS-only artifact in the release pipeline. The old DNS-only plist referenced `no.sybr.supermanager.helper`, unlike the actual `com.sybr.supermanager.helper`; this candidate does not enable a new DNS cleanup service.

### Evidence and validation

- **15 Python tests passed**, including actual transaction scripts operating on temporary paths with mocked root identity/signing/launchd: activation failure, process termination, recovery, payload tampering, concurrent publication, stale build-floor checks, downgrade/same-version rejection and legacy-state refusal. Runtime tests cover changed/extra/missing files, symlinks, writable files, interpreter changes, external libraries and path traversal.
- **83 helper Rust tests passed** with `dev-rpc`; selected helper all-target Clippy passed with the preceding lint policy. A release helper was built, Developer-ID signed and successfully executed with the non-privileged `--version` path.
- **Three real signed-client identity cases passed** through the kernel audit-token/Security.framework authorization code: the pinned client was accepted, wrong bundle ID was rejected, and lack of hardened runtime was rejected. These are deliberately unprivileged fixtures, not a claim that the actual GUI was tested against an installed root helper.
- The macOS app and test bundle compiled (`build-for-testing`) in an isolated unsigned project. No app launch or live service installation occurred. The test fixture selects matching Xcode compiler/SDK explicitly after detecting a CLI/Xcode SDK mismatch.
- A complete ARM64 runtime was compiled from the pinned sources and passed the validator after the packaging corrections. Its six native files were inspected for architecture and dependencies. The signed helper/Bash/wg/wireguard-go/OpenVPN version commands ran successfully. `swanctl --help` ran with the staged strongSwan configuration and reported its compiled crypto plugins. `swanctl --version` requires a live VICI endpoint and was not treated as a passing lifecycle test.
- A **Developer ID Installer-signed ARM64 package was built**, expanded without installation, and inspected for root ownership, expected payload/scripts and final payload hashes. This is an unnotarized test candidate, not a published production release. Binary artifacts are intentionally outside Git under `/tmp/supermanager-sixth-artifacts/`.
- The WireGuard-Go archive initially failed the published Homebrew SHA-256 pin. The replacement hash was accepted only after verifying the upstream tag with the author's PGP key and comparing **all** archive files byte-for-byte against the signed Git tree, with no extra/missing files. The lock retains the old/new hashes and signature/commit evidence. See `installer/system/README.md` for primary-source links and the full rollout contract.

Logs: `/tmp/supermanager-sixth-package-tests.log`, `/tmp/supermanager-sixth-identity-tests.log`, `/tmp/supermanager-sixth-helper-tests.log`, `/tmp/supermanager-sixth-clippy.log`, `/tmp/supermanager-sixth-xcode-final.log`, `/tmp/supermanager-sixth-runtime-build-final.log`, `/tmp/supermanager-sixth-bash-rebuild.log`, `/tmp/supermanager-sixth-package-build-final.log`. The runtime build log includes the intermediate validator failure; the final standalone validator and subsequent package construction passed after the Bash-path and manifest-path corrections.

### Still required before rollout

No disposable test host has been supplied. **Actual signed GUI-to-installed-helper authorization, real package installation/upgrade/recovery, VPN handshake/traffic/DNS/routes, concurrent profiles, crash/sleep/wake/reboot recovery, Intel builds and legacy migration remain unverified.** The test package was not installed, notarized, uploaded as a release or merged into main. Old manual installations deliberately require a separate migration procedure rather than trusting an unknown build floor.

This is process-interruption recovery, not a power-loss-atomic filesystem transaction. Retained backups may contain secrets and need private retention/cleanup. Root runtime ownership does not create customer-level operation authorization. OpenVPN3/Azure and the previously rejected WireGuard/OpenVPN directives remain unsupported. The candidate package does not remove every installation/update/privilege or VPN lifecycle finding from the audit. Follow the concrete live-test release gate in `installer/system/README.md` before deploying it.

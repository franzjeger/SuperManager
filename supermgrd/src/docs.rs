...
pub fn render_customer_doc(
    ...
    // ── SSH hosts ─────────────────────────────────────────────────────
    let mut matched_hosts: Vec<&Host> = hosts
        .iter()
...
pub fn list_customers(profiles: &[Profile], hosts: &[Host]) -> Vec<String> {
...
mod tests {
    ...
    fn sample_host(label: &str, customer: &str, dt: DeviceType) -> Host {
        Host {
            id: Uuid::new_v4(),
            label: label.into(),
